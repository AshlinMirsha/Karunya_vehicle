-- Migration: Update get_today_bus_session_stats to support NULL p_bus_id for Admin (all buses)
...
BEGIN
  ...
  -- Check if Morning / Evening sessions exist today
  SELECT 
    bool_or(s.session_type = 'Morning'),
    bool_or(s.session_type = 'Evening')
  INTO has_morning, has_evening
  FROM public.attendance_sessions s
  WHERE (p_bus_id IS NULL OR s.bus_id = p_bus_id) AND (s.created_at AT TIME ZONE 'Asia/Kolkata')::date = today_ist;
