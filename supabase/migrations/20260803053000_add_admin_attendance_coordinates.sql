drop function public.admin_attendance_sheet();

create function public.admin_attendance_sheet()
returns table (
  student_id uuid,
  full_name text,
...
begin
  ...
  select attendance.student_id, profile.full_name, profile.register_number, attendance.status,
    attendance.checked_in_at, session.session_type, bus.bus_number, attendance.latitude, attendance.longitude
