...
test('attendance API remains JWT-protected', async () => {
  ...
  assert.match(api, /endsWith\('@karunya\.edu'\)/);
  ...
  assert.match(api, /action === 'import-students-csv'/);
  assert.match(api, /action === 'get-admin-activities'/);
...
test('45 students are assigned to Bus No. 3', async () => {
  const migration = await read('supabase/migrations/20260902020000_assign_bus_three_students.sql');
  assert.match(migration, /navaneethakrishnan26@karunya\.edu\.in/);
  assert.match(migration, /sridharshika@karunya\.edu\.in/);
  assert.match(migration, /bus_number = '3'/);
  assert.match(migration, /pending_student_assignments/);
});

test('43 students are assigned to Bus No. 4', async () => {
  const migration = await read('supabase/migrations/20260902030000_assign_bus_four_students.sql');
  assert.match(migration, /annalakshmia@karunya\.edu\.in/);
  assert.match(migration, /kirans26@karunya\.edu\.in/);
  assert.match(migration, /bus_number = '4'/);
  assert.match(migration, /pending_student_assignments/);
});
