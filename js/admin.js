...
const downloadAttendanceSheet = (records) => {
  const header = ['Student', 'Register number', 'Bus number', 'Session', 'Latitude', 'Longitude', 'Checked in', 'Status'];
...
export async function initAdminDashboard() {
  ...
  const attendanceRows = attendance.map((record) => [record.full_name ?? 'Unknown student', record.register_number ?? '—', record.bus_number ?? '—', record.session_type ?? '—', formatCoordinate(record.latitude), formatCoordinate(record.longitude), new Date(record.checked_in_at).toLocaleString('en-IN'), record.status]);
  ...
  renderRows('admin-attendance-list', attendanceRows.map(makeRow), 8, 'No attendance has been recorded yet.');
