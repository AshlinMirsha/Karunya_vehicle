...
export async function initStudentDashboard() {
  ...
  const { data: attendanceRecords, error: attendanceError } = await supabase.from('attendance').select('session_id,status,checked_in_at').eq('student_id', user.id).order('checked_in_at', { ascending: false }).limit(25);
  ...
  const { records, error: detailsError } = await loadAttendanceDetails(attendanceRecords ?? []);
  if (detailsError) { history.innerHTML = '<tr><td colspan="5" class="text-center text-danger py-3">Attendance session details could not be loaded.</td></tr>'; return; }
  history.innerHTML = records.length ? records.map((record) => `<tr><td class="font-monospace">${escapeHtml(new Date(record.checked_in_at).toLocaleString('en-IN'))}</td><td>${escapeHtml(record.session?.session_type)}</td><td class="fw-semibold text-info">${escapeHtml(record.bus?.bus_number)}</td><td><small class="text-white-50">Verified</small></td><td><span class="badge-status badge-present">${escapeHtml(record.status)}</span></td></tr>`).join('') : '<tr><td colspan="5" class="text-center text-white-50 py-3">No attendance has been recorded yet.</td></tr>';
}
